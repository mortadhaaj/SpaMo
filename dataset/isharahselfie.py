...
class IsharahSelfie(torch.utils.data.Dataset):
    """
    Dataset class for the IsharahSelfie sign language dataset.
    ...
    def __init__(
        ...
        """
        Initialize the IsharahSelfie dataset.
        ...
        """
        super().__init__()
        self.csv_ann_file = Path(csv_ann_file)
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        file_id = data.video_pth.rsplit('/', 1)[-1].split('.')[0] #data['fileid']
        pixel_value = None
        glor_value = None
        ...
        result = {
            ...
            'bool_mask_pos': None,
            'text': self._normalize_text(data.sentence),
            'gloss': None, #data['gloss'],
            'id': file_id,
            'num_frames': len(pixel_value) if pixel_value is not None else 0,
            'vid_path': str(self.vid_root / data.video_pth.rsplit('/', 1)[-1]),
            'lang': 'Arabic'
        }
